...
2. Pipe the completion summary markdown into `archetipo spec review {US-CODE}` from `data.project_root`. This single command transitions the spec to `{config.workflow.statuses.review}` AND posts the comment on the parent issue (or silently ignores it for connectors without comment support — never branch on connector type).
